...
func seed(ctx context.Context, tx pgx.Tx) error {
	statements := []struct {
		...
		{"permissions", `INSERT INTO role_permissions(tenant_id,role_id,permission_code) SELECT $1,$2,code FROM permissions WHERE code IN ('sales.read','sales.complete','sales.reverse','customers.read','products.read','mobile.devices.enroll','mobile.devices.read','mobile.devices.manage','mobile.sales.sync','mobile.reconciliation.read','mobile.reconciliation.resolve') ON CONFLICT DO NOTHING`, []any{tenantID, roleID}},
