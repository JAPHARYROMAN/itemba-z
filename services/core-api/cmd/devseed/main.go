...
func seed(ctx context.Context, tx pgx.Tx) error {
	statements := []struct {
		...
		{"permissions", `INSERT INTO role_permissions(tenant_id,role_id,permission_code) SELECT $1,$2,code FROM permissions WHERE code IN ('sales.read','sales.complete','sales.reverse','customers.read','products.read','mobile.devices.enroll','mobile.sales.sync') ON CONFLICT DO NOTHING`, []any{tenantID, roleID}},
		...
		{"authoritative device cache versions", `UPDATE mobile_devices AS d SET master_data_version=c.master_data_version,price_version=c.price_version FROM legal_companies AS c WHERE d.tenant_id=$1 AND d.company_id=$2 AND d.id=$3 AND c.tenant_id=d.tenant_id AND c.id=d.company_id`, []any{tenantID, companyID, deviceID}},
