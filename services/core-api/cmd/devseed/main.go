...
func seed(ctx context.Context, tx pgx.Tx) error {
	statements := []struct {
		...
		{"permissions", `INSERT INTO role_permissions(tenant_id,role_id,permission_code) SELECT $1,$2,code FROM permissions WHERE code IN ('sales.read','sales.complete','sales.reverse','customers.read','customers.accounts.read','customers.credit.manage','customers.collections.post','products.read','mobile.devices.enroll','mobile.devices.read','mobile.devices.manage','mobile.sales.sync','mobile.reconciliation.read','mobile.reconciliation.resolve','operations.read','sales.orders.manage','purchases.requests.manage','purchases.orders.manage','purchases.receive','purchases.invoices.post','purchases.payments.post','inventory.transfers.manage','inventory.counts.manage','inventory.adjustments.post','inventory.planning.read','inventory.planning.manage','inventory.planning.approve','inventory.lots.manage','finance.bank.read','finance.bank.import','finance.bank.match','finance.bank.reconcile','finance.journals.read','finance.journals.manage','finance.journals.post','finance.cash.transfer','finance.bank.adjust','finance.periods.read','finance.periods.close','finance.periods.reopen','finance.accounts.read','finance.accounts.manage','finance.accounts.approve','finance.budgets.read','finance.budgets.manage','finance.budgets.approve','finance.assets.read','finance.assets.manage','finance.assets.approve','finance.assets.depreciate','finance.assets.dispose','finance.treasury.read','finance.treasury.manage','finance.treasury.approve','finance.treasury.transact','finance.intercompany.read','finance.intercompany.manage','finance.intercompany.approve','finance.consolidation.read','reports.financial.read','reports.financial.export','masterdata.read','masterdata.suppliers.manage','masterdata.products.manage','masterdata.approve','purchases.sourcing.read','purchases.sourcing.manage','purchases.sourcing.approve','hr.workforce.read','hr.shifts.manage','hr.shifts.approve','hr.documents.manage','hr.payroll.export') ON CONFLICT DO NOTHING`, []any{tenantID, roleID}},
		{"dashboard permission", `INSERT INTO role_permissions(tenant_id,role_id,permission_code) VALUES($1,$2,'dashboard.read') ON CONFLICT DO NOTHING`, []any{tenantID, roleID}},
		{"audit permission", `INSERT INTO role_permissions(tenant_id,role_id,permission_code) VALUES($1,$2,'audit.read') ON CONFLICT DO NOTHING`, []any{tenantID, roleID}},
		{"security governance permissions", `INSERT INTO role_permissions(tenant_id,role_id,permission_code) SELECT $1,$2,code FROM permissions WHERE code IN ('security.access.read','security.access.manage','security.access.review','security.emergency.activate') ON CONFLICT DO NOTHING`, []any{tenantID, roleID}},
		{"privacy governance permissions", `INSERT INTO role_permissions(tenant_id,role_id,permission_code) SELECT $1,$2,code FROM permissions WHERE code IN ('privacy.cases.read','privacy.cases.manage','privacy.holds.manage','privacy.retention.manage') ON CONFLICT DO NOTHING`, []any{tenantID, roleID}},
		{"integration governance permissions", `INSERT INTO role_permissions(tenant_id,role_id,permission_code) SELECT $1,$2,code FROM permissions WHERE code IN ('integrations.read','integrations.manage','integrations.replay') ON CONFLICT DO NOTHING`, []any{tenantID, roleID}},
		{"role scope", `INSERT INTO user_role_scopes(id,tenant_id,user_id,role_id,company_id,branch_id,warehouse_id) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT DO NOTHING`, []any{roleScopeID, tenantID, operatorID, roleID, companyID, branchID, warehouseID}},
		...
		{"bank account", `INSERT INTO bank_accounts(id,tenant_id,company_id,branch_id,warehouse_id,code,name,account_type,currency,gl_account_id,active) VALUES($1,$2,$3,$4,$5,'CRDB-TZS','CRDB current account DEV','BANK','TZS','bank-current',true) ON CONFLICT(id) DO UPDATE SET name=EXCLUDED.name,active=true`, []any{bankAccountID, tenantID, companyID, branchID, warehouseID}},
		{"chart of accounts", `INSERT INTO gl_accounts(record_id,tenant_id,company_id,id,code,name,account_type,control_account,allow_manual_posting,status,created_at) SELECT gen_random_uuid(),$1,$2,id,id,name,account_type,control_account,allow_manual_posting,'ACTIVE','2020-01-01T00:00:00Z' FROM (VALUES ('receivable','Trade receivables','ASSET',true,false),('tax-payable','Tax payable','LIABILITY',true,false),('cash-on-hand','Cash on hand','ASSET',false,false),('mobile-money-clearing','Mobile money clearing','ASSET',false,false),('bank-card-clearing','Bank card clearing','ASSET',false,false),('bank-current','Bank current','ASSET',false,false),('revenue','Sales revenue','REVENUE',false,false),('cogs','Cost of goods sold','EXPENSE',false,false),('inventory','Inventory','ASSET',true,false),('grni','Goods received not invoiced','LIABILITY',true,false),('payable','Trade payables','LIABILITY',true,false),('inventory-adjustment','Inventory adjustment','EXPENSE',false,true),('stock-in-transit','Stock in transit','ASSET',true,false),('suspense','Suspense','ASSET',false,true)) AS accounts(id,name,account_type,control_account,allow_manual_posting) ON CONFLICT DO NOTHING`, []any{tenantID, companyID}},
		{"posting mappings", `INSERT INTO posting_mappings(id,tenant_id,company_id,mapping_key,account_id,effective_from,status,reason,created_at) SELECT gen_random_uuid(),$1,$2,mapping_key,account_id,'2020-01-01T00:00:00Z','ACTIVE','Development posting configuration','2020-01-01T00:00:00Z' FROM (VALUES ('SALES_RECEIVABLE','receivable'),('SALES_TAX_PAYABLE','tax-payable'),('PAYMENT_CASH','cash-on-hand'),('PAYMENT_MOBILE_MONEY','mobile-money-clearing'),('PAYMENT_BANK_CARD','bank-card-clearing'),('PAYMENT_BANK_TRANSFER','bank-current'),('PROCUREMENT_GRNI','grni'),('PROCUREMENT_PAYABLE','payable'),('INVENTORY_ADJUSTMENT','inventory-adjustment'),('STOCK_IN_TRANSIT','stock-in-transit')) AS mappings(mapping_key,account_id) ON CONFLICT DO NOTHING`, []any{tenantID, companyID}},
