...
Core ERP functional coverage is tracked at **91.7%** in [the coverage assessment](docs/implementation/core-functional-coverage.md). This is not production readiness: statutory integrations, production migrations, infrastructure evidence, professional validation, and the formal release gates remain tracked in [the implementation status](docs/implementation/current-status.md).
