import type { ChangeMobileDeviceStatusCommand } from "@/live-api/types";
...
export async function POST(request: Request, { params }: { params: Promise<{ deviceId: string }> }) {
  try {
    ...
    const command = await requireJsonBody<ChangeMobileDeviceStatusCommand>(request);
    const { deviceId } = await params;
    const repository = await createServerRepository();
    return liveResponse(await repository.changeManagedDeviceStatus(deviceId, command, idempotencyKey));
  } catch (error) {
