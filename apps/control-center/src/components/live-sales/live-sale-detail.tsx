...
function ScopedLiveSaleDetail({ workspace }: { workspace: SaleDetailWorkspace }) {
  ...
  async function submitReversal() {
    ...
    try {
      ...
      const detail = error instanceof PendingCommandConflictError
        ...
        : error instanceof PendingCommandStorageError
          ? error.message
          : "The idempotency key could not be preserved, so the reversal was not sent.";
    ...
    try {
      ...
      setProblem({ type: "about:blank", title: "Connection interrupted", status: 503, code: "bff_unavailable", detail: "Retry is safe: the same reversal reason retains its idempotency key." });
    } finally {
      setSubmitting(false);
    }
  }
