...
function riskLabel(risk: ScheduleCreditPolicyCommand["risk_status"], l: ReturnType<typeof useLanguage>["l"]): string {
  if (risk === "WATCH") return l(text("Monitor closely", "Fuatilia kwa karibu"));
  if (risk === "HOLD") return l(text("Credit on hold", "Mkopo umesimamishwa"));
  return l(text("Standard", "Kawaida"));
}
...
export function CustomerAccountDetail({ workspace }: { workspace: CustomerAccountWorkspace }) {
  ...
  const [limitTzs, setLimitTzs] = useState(minorUnitsToMajorInput(account.active_policy.credit_limit_minor));
  ...
  const parsedLimit = parseMajorUnitsToMinor(limitTzs), termDays = Number(terms), overdueDays = Number(overdue);
  const valid = parsedLimit !== null && Number.isInteger(termDays) && termDays >= 0 && termDays <= 365 && Number.isInteger(overdueDays) && overdueDays >= 0 && overdueDays <= 3650 && reason.trim().length >= 8 && Boolean(effectiveFrom) && (enabled || parsedLimit === 0);

  async function schedulePolicy() {
    if (!valid) return;
    if (parsedLimit === null) return;
    const command: ScheduleCreditPolicyCommand = { credit_enabled: enabled, credit_limit_minor: parsedLimit, payment_terms_days: termDays, max_overdue_days: overdueDays, risk_status: risk, reason: reason.trim(), effective_from: new Date(effectiveFrom).toISOString() };
  ...
  return <div className="page-stack customer-account-page">
    ...
    <section className="detail-hero"><div><div className="heading-badges"><LiveBadge context={context} /><span className={`reconciliation-status status-${account.active_policy.risk_status.toLowerCase()}`}>{riskLabel(account.active_policy.risk_status, l)}</span></div><h1>{account.customer.name}</h1><p>{account.customer.code} · {account.customer.active ? l(text("Active", "Hai")) : l(text("Inactive", "Haifanyi kazi"))}</p></div><div className="detail-actions"><strong>{formatMinorUnits(account.aging.calculated_exposure_minor, context.currency, locale)}</strong><small>{l(text("authoritative exposure", "mfiduo rasmi"))}</small></div></section>
    ...
    <div className="detail-layout"><div className="detail-main"><section className="card"><div className="card-heading"><div><p className="eyebrow">{l(text("Open receivables", "Madeni wazi"))}</p><h2>{l(text("Invoice-level evidence", "Ushahidi wa ankara"))}</h2></div><FileText size={19} /></div><div className="table-scroll"><table className="ledger-table"><thead><tr><th>{l(text("Document", "Hati"))}</th><th>{l(text("Due", "Tarehe ya malipo"))}</th><th>{l(text("Outstanding", "Salio"))}</th></tr></thead><tbody>{account.open_items.map((item) => <tr key={item.id}><td>{item.kind}<small className="table-subline">{item.source_type}</small></td><td>{item.due_at ? formatTimestamp(item.due_at, locale, context.timezone) : "—"}</td><td>{formatMinorUnits(item.outstanding_minor, item.currency, locale)}</td></tr>)}</tbody></table>{account.open_items.length === 0 ? <div className="empty-table"><ShieldCheck size={24} /><strong>{l(text("No open receivables", "Hakuna madeni wazi"))}</strong></div> : null}</div></section></div>
      ...
      {canManage ? <section className="card policy-form"><div className="card-heading"><div><p className="eyebrow">{l(text("Controlled change", "Badiliko linalodhibitiwa"))}</p><h2>{l(text("Schedule policy", "Panga sera"))}</h2></div><CircleAlert size={19} /></div><div className="form-grid"><label><span>{l(text("Credit enabled", "Mkopo umeruhusiwa"))}</span><select value={enabled ? "yes" : "no"} onChange={(event) => { const next = event.target.value === "yes"; setEnabled(next); if (!next) setLimitTzs("0.00"); }}><option value="yes">{l(text("Yes", "Ndiyo"))}</option><option value="no">{l(text("No", "Hapana"))}</option></select></label><label><span>{l(text("Limit (TZS)", "Kikomo (TZS)"))}</span><input inputMode="decimal" value={limitTzs} onChange={(event) => setLimitTzs(event.target.value)} /></label><label><span>{l(text("Payment terms", "Muda wa malipo"))}</span><input type="number" min="0" max="365" value={terms} onChange={(event) => setTerms(event.target.value)} /></label><label><span>{l(text("Overdue tolerance", "Uvumilivu wa kuchelewa"))}</span><input type="number" min="0" max="3650" value={overdue} onChange={(event) => setOverdue(event.target.value)} /></label><label><span>{l(text("Risk", "Hatari"))}</span><select value={risk} onChange={(event) => setRisk(event.target.value as typeof risk)}><option value="STANDARD">{l(text("Standard", "Kawaida"))}</option><option value="WATCH">{l(text("Monitor closely", "Fuatilia kwa karibu"))}</option><option value="HOLD">{l(text("Credit on hold", "Mkopo umesimamishwa"))}</option></select></label><label><span>{l(text("Effective from", "Inaanza"))}</span><input type="datetime-local" value={effectiveFrom} onChange={(event) => setEffectiveFrom(event.target.value)} /></label><label className="field-wide"><span>{l(text("Approval reason", "Sababu ya idhini"))}</span><textarea rows={3} maxLength={500} value={reason} onChange={(event) => setReason(event.target.value)} /></label></div><button className="primary-button" disabled={!valid || submitting} onClick={schedulePolicy}>{submitting ? l(text("Recording…", "Inarekodi…")) : l(text("Schedule policy", "Panga sera"))}</button>{problem ? <div className="sale-problem"><TriangleAlert size={16} /><span><strong>{l(text("Policy was not recorded", "Sera haikurekodiwa"))}</strong>{problem.detail}<details><summary>{l(text("System details", "Maelezo ya mfumo"))}</summary><small>{problem.code}</small></details></span></div> : null}</section> : <section className="permission-empty"><LockKeyhole size={22} /><strong>{l(text("Credit management permission required", "Ruhusa ya kusimamia mikopo inahitajika"))}</strong></section>}</aside></div>
  </div>;
}
