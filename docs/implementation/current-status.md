...
- Add versioned historical product, price, and tax facts plus governed operator recovery/reconciliation for offline cash collected before authoritative configuration drift. Catalog pagination and activation are now pinned to an immutable publication token, but the current slice still fails later historical drift closed for review rather than claiming automatic continuity.
