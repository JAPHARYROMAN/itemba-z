...
- Select and integrate the production identity provider and login/session lifecycle; deploy the API and clients behind managed TLS, provision production devices, and add governed operator workflows for device suspension, allocation changes, queue support, and a Control Center reconciliation evidence screen over the implemented API.
