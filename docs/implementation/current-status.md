...
- Bind catalog download to acknowledgement with an immutable as-of token/content fingerprint (or activation-time version publication). The current four-hour, transition-bounded lease and tax-write drain guard close known scheduling and renewal failures but do not make the download/activation race or statutory offline workflow production-safe.
- Add versioned historical product, price, and tax snapshots plus a consistent paginated download and governed operator recovery/reconciliation for offline cash collected before authoritative configuration drift. The current slice fails such drift closed for review rather than claiming automatic continuity.
